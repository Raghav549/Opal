...
const verify=async()=>{if(verifying)return;setMessage('');if(!/^\\d{6}$/.test(otp)){setMessage('Enter the 6-digit OTP.');return}setVerifying(true);try{const client=await ensureSupabase();if(!client){setMessage('Auth is unavailable.');return}const{data,error}=await client.auth.verifyOtp({email,token:otp,type:'email'});if(error){setMessage(error.message);return}const u={email:data.user?.email||email,name:name||data.user?.user_metadata?.name||'Opal customer'};setAccount(u);setMode('profile')}catch(e){setMessage(e instanceof Error?e.message:'Could not verify OTP.')}finally{setVerifying(false)}};
...
const verify=async()=>{if(busy)return;setMessage('');if(!/^\d{6}$/.test(data.otp))return setMessage('Enter the 6-digit OTP.');setBusy(true);try{const client=await ensureSupabase();if(!client){setMessage('Auth is unavailable.');return}const{error}=await client.auth.verifyOtp({email:data.email,token:data.otp,type:'email'});if(error)setMessage(error.message);else setStep(2)}catch(e){setMessage(e instanceof Error?e.message:'Could not verify OTP.')}finally{setBusy(false)}};
