...
function Orders(){const[orders,setOrders]=useState<any[]>([]);const[loading,setLoading]=useState(true);useEffect(()=>{(async()=>{const client=await ensureSupabase();if(!client){setLoading(false);return}const{data:{user}}=await client.auth.getUser();if(!user){setLoading(false);return}const{data}=await client.from('opal_orders').select('id,status,payment_status,total,currency,created_at,opal_order_items(product_name,variant_name,quantity,line_total)').order('created_at',{ascending:false});setOrders(data||[]);setLoading(false)})()},[]);return <Page title="Orders" kicker="ACCOUNT"><div className="ordersPage"><div className="orderIntro"><span className="mini">ORDER HISTORY</span><h2>Your recent activity.</h2><p>Live orders and their current status are loaded from Opal.</p></div>{loading?<div className="emptyState"><h3>Loading orders…</h3></div>:orders.length?<div className="orderList">{orders.map((o:any)=><div className="orderCard" key={o.id}><div><span className="mini">{String(o.status).toUpperCase()}</span><h3>Order {o.id.slice(0,8)}</h3><p>{new Date(o.created_at).toLocaleString('en-IN')} · {o.payment_status} · ₹{Number(o.total).toLocaleString('en-IN')}</p>{(o.opal_order_items||[]).map((x:any)=><p key={x.product_name+x.variant_name}>{x.product_name} · {x.variant_name} × {x.quantity}</p>)}</div></div>)}</div>:<div className="emptyState"><h3>No orders yet.</h3></div>}</div></Page>}
function Account({account,setAccount}:{account:any,setAccount:(x:any)=>void}){const[email,setEmail]=useState(account?.email||'');const[otp,setOtp]=useState('');const[name,setName]=useState(account?.name||'');const[mode,setMode]=useState(account?'profile':'signin');const[message,setMessage]=useState('');const sendOtp=async()=>{setMessage('');if(!email||!email.includes('@')){setMessage('Enter a valid email.');return}const client=await ensureSupabase();if(!client){setMessage('Auth is unavailable.');return}const{error}=await client.auth.signInWithOtp({email,options:{shouldCreateUser:true}});if(error)setMessage(error.message);else setMode('otp')};const verify=async()=>{setMessage('');const client=await ensureSupabase();if(!client)return setMessage('Auth is unavailable.');const{data,error}=await client.auth.verifyOtp({email,token:otp,type:'email'});if(error){setMessage(error.message);return}const u={email:data.user?.email||email,name:name||data.user?.user_metadata?.name||'Opal customer'};setAccount(u);setMode('profile')};return <Page title="Account" kicker={mode==='otp'?'VERIFY EMAIL':'OPAL'}><div className="accountPage">{mode==='signin'&&<div className="authCard"><h2>Sign in</h2><Field label="EMAIL" value={email} onChange={setEmail} placeholder="you@example.com"/>{message&&<p className="authMessage">{message}</p>}<button className="darkBtn" onClick={sendOtp}>Send OTP</button></div>}{mode==='otp'&&<div className="authCard"><h2>Verify email</h2><Field label="OTP" value={otp} onChange={setOtp} placeholder="6-digit code"/>{message&&<p className="authMessage">{message}</p>}<div className="authActions"><button className="darkBtn" onClick={verify}>Verify & continue</button><button className="ghost3d" onClick={sendOtp}>Resend OTP</button></div></div>}{mode==='profile'&&<div className="accountCards"><div><span className="mini">PROFILE</span><h3>Personal details</h3><Field label="NAME" value={name} onChange={setName}/><Field label="EMAIL" value={email} onChange={setEmail}/><button className="darkBtn" onClick={()=>setAccount({email,name:name||'Opal customer'})}>Save changes</button></div><div><span className="mini">ORDERS</span><h3>Order history</h3><p>Orders, status events and tracking will appear here.</p><button className="ghost3d" onClick={()=>location.hash='orders'}>View orders</button></div></div>}</div></Page>}
...
function Checkout({items,onDone}:{items:any[],onDone:()=>void}){const[step,setStep]=useState(0);const[message,setMessage]=useState('');const[data,setData]=useState({email:'',otp:'',name:'',house1:'',house2:'',flat:'',pin:'',district:'',state:'',payment:'cod'});const labels=['Email','Verification','Name','House 1','House 2','Flat / No.','PIN & District','State','Payment','Confirm'];const set=(k:string,v:string)=>setData(d=>({...d,[k]:v}));const total=items.reduce((n,x)=>n+x.price,0);const sendOtp=async()=>{setMessage('');if(!data.email.includes('@'))return setMessage('Enter a valid email.');const client=await ensureSupabase();if(!client)return setMessage('Auth is unavailable.');const{error}=await client.auth.signInWithOtp({email:data.email,options:{shouldCreateUser:true}});if(error)setMessage(error.message);else setStep(1)};const verify=async()=>{setMessage('');const client=await ensureSupabase();if(!client)return setMessage('Auth is unavailable.');const{error}=await client.auth.verifyOtp({email:data.email,token:data.otp,type:'email'});if(error)setMessage(error.message);else setStep(2)};const place=async()=>{setMessage('');const client=await ensureSupabase();if(!client)return setMessage('Store backend unavailable.');const{data:{user}}=await client.auth.getUser();if(!user)return setMessage('Please verify your email first.');if(!items.length)return setMessage('Your hold is empty.');const{data:order,error}=await client.from('opal_orders').insert({user_id:user.id,email:user.email||data.email,customer_name:data.name||'Opal customer',status:'pending',payment_method:data.payment,payment_status:'pending',total,currency:'INR',address:{house1:data.house1,house2:data.house2,flat:data.flat,pin:data.pin,district:data.district,state:data.state}}).select('id').single();if(error||!order)return setMessage(error?.message||'Could not create order.');const rows=[];for(const item of items){const{data:p}=await client.from('opal_products').select('id').eq('name',item.name).single();const{data:v}=p?await client.from('opal_variants').select('id').eq('product_id',p.id).eq('name',item.variant).single():{data:null};rows.push({order_id:order.id,variant_id:v?.id||null,product_name:item.name,variant_name:item.variant,unit_price:item.price,quantity:1})}const{error:itemError}=await client.from('opal_order_items').insert(rows);if(itemError){await client.from('opal_orders').delete().eq('id',order.id);return setMessage(itemError.message)}await client.functions.invoke('opal-email',{body:{type:'order_received',order_id:order.id}}).catch(()=>null);onDone()};return <Page title="Checkout" kicker={labels[step].toUpperCase()}><div className="checkoutFlow">{step===0&&<><Field label="EMAIL" value={data.email} onChange={v=>set('email',v)} placeholder="you@example.com"/><button className="darkBtn" onClick={sendOtp}>Send verification OTP</button></>}{step===1&&<><Field label="OTP" value={data.otp} onChange={v=>set('otp',v)} placeholder="6-digit code"/><div className="authActions"><button className="darkBtn" onClick={verify}>Verify email</button><button className="ghost3d" onClick={sendOtp}>Resend OTP</button></div></>}{step===2&&<Field label="NAME" value={data.name} onChange={v=>set('name',v)} placeholder="Full name"/>}{step===3&&<Field label="HOUSE 1" value={data.house1} onChange={v=>set('house1',v)} placeholder="House / building"/>}{step===4&&<Field label="HOUSE 2" value={data.house2} onChange={v=>set('house2',v)} placeholder="Street / locality"/>}{step===5&&<Field label="FLAT / NO." value={data.flat} onChange={v=>set('flat',v)} placeholder="Flat / floor / plot"/>}{step===6&&<div className="twoFields"><Field label="PIN CODE" value={data.pin} onChange={v=>set('pin',v)} placeholder="PIN"/><Field label="DISTRICT" value={data.district} onChange={v=>set('district',v)} placeholder="District"/></div>}{step===7&&<Field label="STATE" value={data.state} onChange={v=>set('state',v)} placeholder="State / region"/>}{step===8&&<div className="paymentOptions">{[['card','▣','Cards'],['upi','◉','UPI'],['netbanking','⌂','Net banking'],['cod','▤','Cash on delivery']].map(([id,icon,label])=><button className={data.payment===id?'selectedPay':''} key={id} onClick={()=>set('payment',String(id))}><span>{icon}</span><strong>{String(label)}</strong><small>{id==='cod'?'Pay on delivery':'Payment is recorded as pending until a gateway confirms it.'}</small></button>)}</div>}{step===9&&<div className="confirmation"><span className="mini">ORDER CONFIRMATION</span><h2>Ready to place.</h2><p>We'll create a real Opal order record. Online payments remain pending until a payment gateway is connected.</p>{items.map((x,i)=><div className="reviewRow" key={i}><span>{x.name} · {x.variant}</span><b>₹{x.price.toLocaleString('en-IN')}</b></div>)}<div className="reviewTotal"><span>Total</span><b>₹{total.toLocaleString('en-IN')}</b></div><button className="darkBtn" onClick={place}>Confirm order</button></div>}{message&&<p className="authMessage">{message}</p>}{step>=2&&step<9&&<button className="darkBtn stepNext" onClick={()=>setStep(s=>Math.min(s+1,9))}>Continue</button>}</div></Page>}
