...
function Cart({items,onBack,onRemove}:{items:any[],onBack:()=>void,onRemove:(i:number)=>void}){return <Page title="Hold" kicker="BAG"><div className="cartPage">{items.length===0?<p>Your hold is empty.</p>:<><div className="cartRows">{items.map((x,i)=><div className="cartRow" key={i}><img src={x.image} alt=""/><div><strong>{x.name}</strong><small>{x.variant} · ₹{x.price}</small></div><button className="removeHold" onClick={()=>onRemove(i)} aria-label="Remove hold">×</button></div>)}</div><div className="cartSummary"><span>Subtotal</span><b>₹{items.reduce((n,x)=>n+x.price,0).toLocaleString('en-IN')}</b></div><button className="darkBtn" onClick={onBack}>Continue</button></>}</div></Page>}
